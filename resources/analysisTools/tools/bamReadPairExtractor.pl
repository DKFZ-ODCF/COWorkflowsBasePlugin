...
BEGIN {
  ...
  sub usage { # TODO make this nicer with pod2usage
    print "usage: $0 ";
    for (keys %opts) {
      print '[--', $_;
...
while (<$bamfh>) {
  ...
  if ($readHash->{$fields->[0]}) { # pair complete
	...
	if (MINREADLENGTH() > 0 && $seqlength < MINREADLENGTH() && $seqlength2 >= MINREADLENGTH()){
		...
		COUNT && $count{single_read_deleted}++;
      		$hs--;
      		next;
	} elsif (MINREADLENGTH() > 0 && $seqlength < MINREADLENGTH() && $seqlength2 < MINREADLENGTH()){ #both sequences too short, only delete read ID in readHash, no output
      		delete($readHash->{$fields->[0]}); 
		COUNT && $count{read_pair_deleted}++;
      		$hs--;
      		next;
	} else { #both sequences still long enough
...
sub check_selection {
  ...
  #TODO: check validity of input
    #    my $selected = 0;
  if (UNMAPPED && $flags->{UNMAPPED}) { COUNT && $count{unmapped}++; return 1;} #read unmapped
  if (UNMAPPED && $flags->{PAIRED} && $flags->{M_UNMAPPED}) { COUNT && $count{m_unmapped}++; return 1;} #read paired and mate unmapped
  if (NOPROPERPAIR && $flags->{PAIRED} && !$flags->{MAP_PAIR}) { COUNT && $count{no_proper_pair}++;  return 1;} #read paired and both mapped and no proper pair
  if (SOFTCLIPPED && $fields->[5] =~ m/S/) { COUNT && $count{softclipped}++;  return 1;} # read soft-clipped
  if (MQT > 0 && $fields->[4] < MQT) { COUNT && $count{mqt}++;  return 1;} # mapping quality below threshold
  ...
  } else { # the alignment falls into at least two bins; test each bin separately
    DEBUG && say '$a[0] != $a[2]';
...
sub get_flags { # takes integer value from SAM file FLAG field and returns ref to hash with all tags as keys and either 0 or 1 as value
